...
function DetailOrder() {
  ...
  useEffect(() => {
    (async () => {
      if (id_nv) {
        const res = await detailExportInvoiceAPI.getDetailExportInvoice(params.idhdx);
    ...
    <div className="px-4 pb-20">
      ...
      <div>
        ...
        <div className="mt-5">
          ...
          {invoiceData?.map(({ten_sp, hinh_anh, ten_ms, ten_kt, gia_ban_sp, so_luong_xuat}, idx) => (
            <div key={idx} className="flex gap-4 mt-4">
              <img className="w-[120px] h-[120px] rounded-lg" src={hinh_anh} alt="" />
              <div>
                <p>
                ...
                <p>
                  Giá:{" "}
                  <strong>
                    {new Intl.NumberFormat("vi-VN", {style: "currency", currency: "VND"}).format(gia_ban_sp)}
                  </strong>
                </p>
              </div>
            </div>
          ...
          <div className="mt-4">
            ...
            <div className="flex justify-between">
              <p>Phí vận chuyển</p>
              <p>
                {new Intl.NumberFormat("vi-VN", {style: "currency", currency: "VND"}).format(exportInvoice[0]?.tien_vc)}
              </p>
            </div>
            <div className="flex justify-between">
              <p>Tổng cộng</p>
          ...
          <div>
            ...
            {exportInvoice[0]?.trang_thai_gh === "Đang giao hàng" && (
              <button
                onClick={() => successShip(exportInvoice[0]?.id_hdx)}
